...
Later studies showed that this is this fidelity is indeed optimal for the cloning an arbitrary input state. 
...
\begin{center}
\begin{quantikz}
\lstick{$|\psi \rangle_{a_0}$}   &\qw &  \gate[3][2cm]{U_{copy}}&\qw & \rstick{\hspace{-4 mm}$\rho_1$} \\
\lstick{$|0\rangle_{a_1}$} & \gate[2][2cm]{U_{prepare}}  && \qw & \rstick{\hspace{-4 mm}$\rho_2$} \\
...
The angles used to produce an arbitrary state have been previously studied \cite{Buzek1996, Buzek1999}. The following angles were used for the preparation of the state given in Eq. \eqref{eq:uqcm_prep_state}, that is, 
...
\lstick{}   & \ctrl{1}&\ctrl{2}&\targ{}&\targ{}&\qw & \rstick{\hspace{-4 mm}$\rho_1$} \\
\lstick{} &\targ{} & \qw & \ctrl{-1} &\qw&\qw & \rstick{\hspace{-4 mm}$\rho_2$} \\
...
\end{equation}
Then, we calculate the fidelity of a pure state with a mixed state, which is given by,
\begin{equation}
...
\end{equation}
This is the maximum possible fidelity that can be obtained when one tries to copy an arbitrary quantum state. The final state is the same for the input qubit, and for the first copying qubit, where the two clones are made.

In the implementation of the UQCM we must take into account connectivity of the qubits found in each device. If we consider both the preparation of the ancillary qubits and the copying itself, we note that all the three qubits have to be connected to each other. However, this is not always the case in most experimental devices. Thus, we require an additional SWAP gate to connect the qubits that are not physically connected to each other. 
This extra SWAP gate greatly increases the depth of the circuits, since it is implemented using three CNOT gates. The full circuit implemented with the additional SWAP gate can be observed in Figure \ref{circuit:uqcm_exp}.

\begin{figure}[H]
\begin{center}
\begin{quantikz}
    \lstick{$\ket{\psi}_{a_0}$} & \qw & \qw & \qw & \qw & \qw & \swap{2} & \targ{} & \qw & \ctrl{2}  & \qw & \qw &  \\
    \lstick{$\ket{0}_{a_1}$}    & \qw & \targ{} & \gate{R_y(\theta_2)}  & \ctrl{1}  & \qw & \qw & \qw & \targ{} & \qw & \ctrl{1}  & \qw & \rstick{\hspace{-4 mm}$\rho_{2}$}  \\
    \lstick{$\ket{0}_{a_2}$} & \gate{R_y(\theta_1)} & \ctrl{-1}  & \qw & \targ{} & \gate{R_y(\theta_3)}  & \targX{}  & \ctrl{-2} & \ctrl{-1} & \targ{} & \targ{} & \qw & \rstick{\hspace{-4 mm}$\rho_{1}$}
\end{quantikz}.
...
The fact that $\theta_2=0$ is a 'lucky' coincidence. If we wanted to copy the $xy$-equator, all the three rotations would have been necessary. 
...
\begin{figure}[H]
    \centering
        ...
        \caption{Fidelity of the PCQCM plotted on the Bloch sphere. The Mollweide projection \cite{2Dprojection} was used in order to represent the sphere in 2D.}
...
where the controlled-Hadamard can be rewritten using more elementary gates:
...
\begin{figure}[H]
    \begin{minipage}{.5\textwidth}
      ...
      \begin{minipage}{.5\textwidth}
        ...
        \caption{Fidelity of the PCEQCM plotted over the Bloch sphere. The Mollweide projection \cite{2Dprojection} was used in order to represent the sphere in 2D.}
...
More recently, the UQCM has been implemented in a superconducting system \cite{UQCMwithSuperconducting}, reaching a fidelity $F\approx 0.83$ on one qubit and $F\approx 0.79$ on the other.
