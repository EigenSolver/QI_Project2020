...
\]
After some manipulations, it is possible to obtain the following expression (renaming $\theta' \to \theta$ and $\phi' \to \phi$):
\begin{equation}
...
\begin{equation}
    \overline{F}_{xy}=\frac{1}{\int_S \dd{\Omega}}\int_S F_{xy}(\Omega)\dd{\Omega}=\frac{1}{2}\int_0^\pi F_{xy}(\theta,\phi)\sin(\theta)\dd{\theta}.
    =\frac{7+2\sqrt{2}}{12}\approx 0.819.
\end{equation}
...
\begin{center}
    ...
    \lstick{\hspace{-1cm}$\ket{0}_{a_1}$}    & \qw                  &\gate{H}    & \ctrl{-1}    & \gate{R_x(\pi/2)} & \qw & \rstick{\hspace{-4 mm}$\rho_2$} 
    \end{quantikz}
\end{center}
