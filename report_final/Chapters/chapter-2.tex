...
\begin{quantikz}
    ...
    \lstick{$\ket{0}_{a_1}$}    & \qw & \targ{} & \gate{R_y(\theta_2)}  & \ctrl{1}  & \qw & \qw & \qw & \targ{} & \qw & \ctrl{1}  & \qw & \rstick{\hspace{-4 mm}$\rho_{1}$}  \\
    \lstick{$\ket{0}_{a_2}$} & \gate{R_y(\theta_1)} & \ctrl{-1}  & \qw & \targ{} & \gate{R_y(\theta_3)}  & \targX{}  & \ctrl{-2} & \ctrl{-1} & \targ{} & \targ{} & \qw & \rstick{\hspace{-4 mm}$\rho_{2}$}
\end{quantikz}.
