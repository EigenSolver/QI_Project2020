...
We mainly discuss the results corrected with readout calibration. The plots of the results not corrected with readout calibration are reported in Appendix A.

\section{Universal quantum cloning machine}
    
\subsection{Sphere}

In Section \ref{sec:approximateqcm} we found that the expected fidelity for the UQCM is $F=5/6$ for all the points on the Bloch sphere. 
However, we expect the statistical uncertainty discussed Chapter \ref{sec:implementation} because of the finite sampling (we are using $16384$ shots for each input state for Starmon-5 and $8192$ for the IBM backends). The expected statistical uncertainty is (see \ref{eqn:dispersion_uqcm}):
\[
    \sigma_{stat}^{(8192)}\approx 0.004, \quad \sigma_{stat}^{(16384)}\approx 0.003.
\]
The average fidelities and the corresponding dispersions that we found for the difficult backends are summarized in Table \ref{tab:uqcm_results_full_sphere}. These results include readout correction.
Also, the data is represented in a histogram in Figure \ref{fig:hist_uqcm_sphere}. 
...
First of all, we found that the fidelity is lower than the ideal case. From Figure \ref{fig:hist_uqcm_sphere}, we observe that the average fidelity for all the backends is around $0.75$. 
For all the backends the dispersion is also greater than expected. As we mentioned, the only variations should be caused by statistical errors. The obtained deviations are one order of magnitude larger than the expected statistical uncertainty. We attribute this large deviation to the details of the imperfections of each device. 


\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/UQCM/Histograms/histo_sphere.png}
...
From the pictures below, we can observe that the quality of the copies is uniform only in a few cases (e.g. Athens in Figure \ref{fig:uqcm_athens}). 
For most of the other backends, the quality of the copies is different for different regions of the Bloch sphere. For example, it is very clear in Ourense (see Figure \ref{fig:uqcm_our}) that the regions around $\ket{0}$ and $\ket{1}$ (the North and the South pole) are copied better than the region along the $xy$-equator.
It is also possible to observe that the pattern visible for both the copies made on Yorktown (Figure \ref{fig:uqcm_york}) is different from all the others. In this case the better copied regions do not correspond to the poles.

Finally, we observe that something went wrong in the sampling of the Bloch sphere for Valencia (Figure \ref{fig:uqcm_valencia}).
We can observe a strip in the lower hemisphere of the Bloch sphere that is uniformly different.  This strip has fidelities larger than $1$ (for the first copy).
The results for Valencia on the whole Bloch sphere should therefore not be considered \footnote{One might think that this is due to no longer valid readout correction parameters. 
However, the same strip can be seen also in the non corrected results (see Figure \ref{fig:uqcm_valencia_app} in Appendix A).}.

We have seen similar artifacts on other IBM backends also for other QCMs, even if not as significant as for the UQCM on Valencia. They are usually present in both the corrected and non-corrected results.
They correspond to one batch of circuits (on the IBM backends it is possible to group $75$ circuits together).
They could be due to the fact that two subsequent batches of circuits are sometimes run hours apart, depending on the length of the queue.
This would imply that the condition and the calibration of the quantum processors are not the same throughout the sampling.
...
For completeness, we include the results without the readout correction in Table \ref{tab:uqcm_results_full_sphere_non}. It is possible to observe that the readout correction did not change the results significantly.
Note that readout correction does not necessarily improves the results increasing the fidelity, in certain cases it decreases it.
Indeed, implementing readout correction we should expect a results closer to the real output of the machine, which already includes errors from other sources during the execution.
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
    \hline
...
\begin{figure}[H]
    \centering
            \includegraphics[width=0.7\textwidth]{Figures/UQCM/Histograms/histo_sphere_corrected.png}
...
\vspace{-1cm}
\subsection{Equator}
We also sampled the $xz$-equator of the Bloch sphere, in order to compare the results with the PCQCM and the EPCQCM. Theoretically, there should not be any difference between the $xz$-equator and the rest of the Bloch sphere for the UQCM, but this should not be assumed a priori.
The results (readout corrected) are shown in Table \ref{tab:uqcm_results_eq} and Figure \ref{fig:hist_uqcm_eq}. We found that the fidelity has improved with respect to the full sphere case for both copies compared to the full sphere.
Indeed, we do not see any significant difference when comparing these results with the previous ones. 
The oscillatory patterns that can be seen for several backends (e.g. Ourense in Figure \ref{fig:uqcm_eq_our}) are consistent with the patterns visible on the whole Bloch sphere: the regions around $\ket{0}$ and $\ket{1}$ are copied better than the regions around $\ket{+}$ and $\ket{-}$.
A more detailed analysis of such pattern can be found in the results for the PCQCM.
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
    \hline
...
\begin{figure}[H]
    \centering
            \includegraphics[width=0.7\textwidth]{Figures/UQCM/Histograms/histo_equator_corrected.png}
...
For completeness, we have included the results without the readout correction in Table \ref{tab:uqcm_results_eq_non}.
...
\begin{figure}[H]
    \centering
            \includegraphics[width=0.7\textwidth]{Figures/UQCM/Histograms/histo_equator.png}
...
Finally, we report the results considering only the BB84 states as input states, i.e. $\ket{0}$, $\ket{1}$, $\ket{+}$ and $\ket{+}$.
The results are shown in Table \ref{tab:uqcm_results_bb} with readout correction, and in Table \ref{tab:uqcm_results_bb_non} without it.
Looking at the plots of the previous section, it is not surprising that the results do not different significantly from the cloning of the equatorial states.
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/UQCM/Histograms/histo_bb84_corrected.png}
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/UQCM/Histograms/histo_bb84.png}
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/PhaseCovariant/Histograms/histo_sphere_corrected.png}
...
We have discussed such artifacts in the results for the UQCM on the whole sphere.
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/PhaseCovariant/Histograms/histo_sphere.png}
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/PhaseCovariant/Histograms/histo_equator_corrected.png}
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/PhaseCovariant/Histograms/histo_equator.png}
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/PhaseCovariant/Histograms/histo_bb84_corrected.png}
...
We conclude the discussion of the results for the PCQCM with the results for the BB84 states without readout correction.
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
    \hline
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/PhaseCovariant/Histograms/histo_bb84.png}
...
The EPCQCM only requires 2 qubits. Therefore, we could run it also on Spin-2.
\subsection{Sphere}
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/Economical/Histograms/histo_sphere_corrected.png}
...
It is possible to observe that the EPCQCM performs better than UQCM because of the simplicity of the circuit, as we have seen in the PCQCM case. 
In the pictures below it is possible to see that the fidelity on the Bloch sphere varies as expected: it is clearly visible that one hemisphere is copied better than the other.
The only backend for which a slightly different pattern is visible is Yorktown (which had different patterns compared to the other backends also for the other QCMs).

For some backends a fidelity greater than the ideal one is achieved. 
This could be due some systematic inaccuracy in the preparation of the input states or in the measurement of the fidelity of the copies.
For example, the rotations could be skewed towards the better hemisphere, for some hardware inaccuracy. This would lead to an oversampling of such hemisphere and to an undersampling of the other one.
...
Finally, we report the results obtained without readout correction. 
In this case it is possible to observe that for Spin-2 the readout correction changes the results significantly. Indeed, for this backend the readout is more error-prone, possibly because of the different technology (Spin-2 uses spin qubits).
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/Economical/Histograms/histo_sphere.png}
...
The fidelity for the copies of equatorial states using an ideal EPCQCM is:
\[
  F=\frac{1}{2}+\frac{1}{2\sqrt{2}}\approx0.854.
\]
As we have mentioned in the previous section of PCQCM, the statistical dispersion of fidelities should be (see Equation \eqref{eqn:dispersion_epcqcm}): 
\[
  \sigma_{stat}^{(4096)}\approx 0.006, \quad \sigma_{stat}^{(8192)}\approx 0.004, \quad \sigma_{stat}^{(16384)}\approx 0.003.
\]
The results for the equator are shown in Table \ref{tab:results_epcqcm_onlyequator_corrected_corrected} and in Figure \ref{fig:epc_histo_equator_corrected}.
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    \hline
...
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/Economical/Histograms/histo_equator_corrected.png}
...
The average fidelity obtained using the EPQCM on equatorial states is better than that of UQCM and is comparable with PCQCM.
Again, the average fidelities on some of the backends are greater than the theoretical expectation, which should not be a concern as we have explained in the previous section.
It is also possible to observe again the already discussed oscillatory patterns.
The first copy of Valencia is close to the ideal bound, both for the average fidelity and the dispersion of the data. In this case the quality of the copy is really state independent.
For the other backends that do not show an oscillatory pattern, however, the dispersion is still greater than the expected statistical uncertainty. 
Most of the backends are very close to the ideal bound.
...
In Table \ref{tab:results_epcqcm_onlyequator_not_corrected} are shown the results without readout correction. It is possible to observe again that for Spin-2 the readout correction improves the fidelity significantly.
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    \hline
    ...
    \end{tabular}
    \caption{Average fidelity and corresponding standard deviation on the $xz$-equator for the two copies on each backend. The results are not corrected with readout calibration.}
    \label{tab:results_epcqcm_onlyequator_not_corrected}
\end{table}
\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/Economical/Histograms/histo_equator.png}
...
\subsection{BB84 states}
We conclude the results section with the results for the BB84 states. As for the previous QCMs, there is not a significant difference with respect to the cloning of the whole equator.
Hence, the same considerations as for the equator hold. Again, most of the backends are very close to the ideal bound.
In Table \ref{tab:results_epcqcm_bb84_corrected} and Figure \ref{fig:epc_histo_bb84_corrected} are reported the results corrected with readout calibration, 
in Table \ref{tab:results_epcqcm_onlyequator_not_corrected} and Figure \ref{fig:epc_histo_bb84_not_corrected} are reported the non-corrected results.
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{Average fidelity and corresponding standard deviation for the BB84 for the two copies on each backend. The results are corrected with readout calibration.}
    \label{tab:results_epcqcm_bb84_corrected}
\end{table}

\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/Economical/Histograms/histo_bb84_corrected.png}
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{Average fidelity and corresponding standard deviation for the BB84 for the two copies on each backend. The results are not corrected with readout calibration.}
    \label{tab:results_epcqcm_bb84_not_corrected}
\end{table}

\begin{figure}[H]
  \centering
          \includegraphics[width=0.7\textwidth]{Figures/Economical/Histograms/histo_bb84.png}
