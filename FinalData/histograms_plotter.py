...
save_pic_path = "PhaseCovariant/histo_sphere.png"
# Put the title of the picture
title = "PCQCM - Sphere"
# Choose whether to get the readout corrected results or not
readout_corrected = False
# Put the path for IBM averages
path_IBM = "PhaseCovariant/IBM/FullSphere/average_fidelities.txt"
# Put the path for Starmon averages
path_starmon = "PhaseCovariant/Starmon/FullSphere/average_fidelities.txt"
# Put the path for Spin2 averages (just leave this if you are not using it)
path_spin2 = "PhaseCovariant/Spin2/FullSphere/average_fidelities.txt"
...
if readout_corrected:
    dict = {"starmon_corrected": 0, "athens_corrected": 1, "ourense_corrected": 2, "santiago_corrected": 3,
            "valencia_corrected": 4, "vigo_corrected": 5, "ibmqx2_corrected": 6}
else:
    dict = {"starmon": 0, "athens_": 1, "ourense_": 2, "santiago_": 3, "valencia_": 4, "vigo_": 5,
            "ibmqx2_": 6}
...
if use_spin2:
    ...
    if readout_corrected:
        dict = {"spin2_corrected": 0, "starmon_corrected": 1, "athens_corrected": 2, "ourense_corrected": 3,
                "santiago_corrected": 4, "valencia_corrected": 5, "vigo_corrected": 6, "ibmqx2_corrected": 7}
    else:
        dict = {"spin2": 0, "starmon": 1, "athens_": 2, "ourense_": 3, "santiago_": 4, "valencia_": 5,
                "vigo_corrected": 6, "ibmqx2_": 7}
...
# Start filtering to get only the corrected values
...
for i in range(len(name)):
    if name[i] in dict:
        fidelity_copy_1_filtered[dict[name[i]]] = fidelity_copy_1[i]
        stddev_copy_1_filtered[dict[name[i]]] = stddev_copy_1[i]
        fidelity_copy_2_filtered[dict[name[i]]] = fidelity_copy_2[i]
        stddev_copy_2_filtered[dict[name[i]]] = stddev_copy_2[i]
...
if error_bars_active:
    rects1 = ax.bar(x - width / 2, fidelity_copy_1_filtered, width, label='Copy 1',
                    yerr=stddev_copy_1_filtered / np.sqrt(npoints), capsize=10)
    rects2 = ax.bar(x + width / 2, fidelity_copy_2_filtered, width, label='Copy 2',
                    yerr=stddev_copy_2_filtered / np.sqrt(npoints), capsize=10)
else:
    rects1 = ax.bar(x - width / 2, fidelity_copy_1_filtered, width, label='Copy 1')
    rects2 = ax.bar(x + width / 2, fidelity_copy_2_filtered, width, label='Copy 2')
